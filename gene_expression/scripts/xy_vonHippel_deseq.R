...
#load data
raw.counts = read.table('readCounts_vonHippel_SNPsplit.tsv', header = T, row.names='geneID', sep="\t")


#isolate the X and Y reads for males
cnames = colnames(raw.counts)
males = cnames[grep("_M", cnames)]
pe.males = males[!grepl('trim', males)] #3 total
se.males = males[grepl('trim', males)] #2 total

#combine the st reads for the paired end reads
counts0=raw.counts[,colnames(raw.counts) %in% pe.males]
...
#format the single-end counts
se.counts = raw.counts[,colnames(raw.counts) %in% se.males]
colnames(se.counts) = sub('.trim', '', colnames(se.counts))
colnames(se.counts) = sub('.counts.txt', '', colnames(se.counts))

#join pe and se
counts = cbind(pe.counts, se.counts)
head(counts)
dim(counts) #two sets of reads for each of the 5 male samples

#double-check that worked
sum(counts$SRR2976063_M.genome2 == counts0$SRR2976063_M.genome2 + counts0$SRR2976063_M.genome2_st) == nrow(counts0)
...
ll=load("~/gitreps/pungitius_sex_chromosome/figure_plotting/maleVfemale_vonHippel.Rdata")
head(s.r)
s.df = data.frame(s.r)
...
sr.vh = s.r
m.vh = m
save(xy.vh, sr.vh, m.vh, file="~/gitreps/pungitius_sex_chromosome/figure_plotting/xy_vonHippel.Rdata")
